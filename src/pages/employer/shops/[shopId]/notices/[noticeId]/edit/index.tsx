...
const EditJobInfo = () => {
      ...
      { shopId, noticeId, data: formData },
      {
        onSuccess: () => {
          ...
          openModal(
            "confirm",
            "공고 수정이 완료되었습니다.",
            () => router.replace(`/employer/shops/${shopId}/notices/${noticeId}`),
            {
  ...
  const handleCloseClick = () => {
    openModal("action", "수정을 취소하시겠습니까?", () => router.push(`/employer/shops/${shopId}/notices/${noticeId}`));
  };
    ...
    if (!isGetPending && jobinfoData) {
      ...
      if (jobinfoData.item.closed) {
        openModal(
          "confirm",
          "마감된 공고는 수정할 수 없습니다.",
          () => router.push(`/employer/shops/${shopId}/notices/${noticeId}`),
          {
            closeOnOverlayClick: false,
            closeOnEsc: false,
          },
